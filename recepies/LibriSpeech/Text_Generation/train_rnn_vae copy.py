...
from lobes.model import SentenceVAE
import yaml
import argparse
import os
...
def run(params_file, device,overfitting_test=False):
    ...
    model = SentenceVAE(hparams['vocab_size'], hparams['embedding_size'], hparams['rnn_type'], hparams['hidden_size'], hparams['word_dropout'], hparams['embedding_dropout'], hparams['latent_size'],
                special_tokens['sos_idx'], special_tokens['eos_idx'], special_tokens['pad_idx'], special_tokens['unk_idx'], hparams['max_sequence_length'], hparams['num_layers'], bidirectional=False)
...
def train_batch(model, batch, device,loss_module,optimizer,step,hparams):
    ...
    preds, mean, logv, z  = model(tokens_bos,tokens_bos_lens)
...
if __name__ == "__main__":
    ...
    run(
        args.params, 'cpu', args.overfitting)